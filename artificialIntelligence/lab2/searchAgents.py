...
def manhattanDist(pointX,pointY):
...
def cornersHeuristic(state, problem):
  ...
  lastPositionLookedAt = None
  
  for i in range (0,4):
    if exploredcorners[i]:        
        mapcorners.remove(corners[i])
    
  while len(mapcorners) > 0:    
    closestDistance = None
    remainingCorners = list(mapcorners)
    for corner in remainingCorners:
        if closestDistance == None or manhattanDist(currentPosition, corner) < closestDistance:
            closestDistance = manhattanDist(currentPosition, corner)
