...
class Circle():
    def __init__(self,surf,x,y,size,s):
        self.control=s
    ...
    def addNew(self):
        self.control.myCirc.append(Circle(self.Surf,START[0],START[1],(10,15),self))
              
    def boundaries(self,pad):
        ...
        elif not (0<self.rect[0] <= WIDTH):
            self.ranAngle(random.randrange(-45,45))
        elif not (0<self.rect[1] <= HEIGHT):
            self.speed[1] *= -1
            
    def boundariesBoolean(self,pad):
        if(pad.top[1]<=self.rect[1] and pad.down[1]>=self.rect[1] and self.rect[0]<=5):
            return 1
...
class Pad():
    ...
    def moveUP(self):  
        if not (self.top[1]<=0):
            self.top[1]-=self.speed
            self.down[1]-=self.speed
...
class Control:
    def __init__(self):
        ...
        self.myCirc = [Circle(self.screen,START[0],START[1],(10,15),self)]
    ...
    def main(self):
        while (True):
            ...
            if(len(self.myCirc)==0):
                        self.myCirc = [Circle(self.screen,START[0],START[1],(10,15),self)]
            for i in self.myCirc[:]:
                    i.update(self.screen,self.myPad)
                    if (i.boundariesBoolean(self.myPad)==1):
                        if(len(self.myCirc[:])<MAXNUMOFBALL): self.myCirc.append(Circle(self.screen,START[0],START[1],(10,15),self))
                        i.update(self.screen,self.myPad)
                    if (i.boundariesBoolean(self.myPad)==2):
                        self.myCirc.remove(i)
